...
// ── Page title map ────────────────────────────────────────────────────────────
// Derives the topbar title + subtitle from the current route.

const PAGE_META: Record<string, { title: string; subtitle: string }> = {
  '/':           { title: 'Dashboard Overview',  subtitle: 'Welcome back, Administrator.'         },
  ...
  '/fees':       { title: 'Fee Records',          subtitle: 'Track collections and dues.'         },
  '/timetable':  { title: 'Academic Timetable',  subtitle: 'Manage schedules across all sections.'},
  '/homework':   { title: 'Homework',            subtitle: 'Assignments across classes.'          },
...
const DEFAULT_META = { title: 'EduNXT Admin', subtitle: '' };
...
export default function Topbar({ onMenuClick }: TopbarProps) {
  ...
  // Match exact first, then prefix (for nested routes like /students/123)
  const meta =
    PAGE_META[pathname] ??
  ...
  return (
    <header className="sticky top-0 z-30 min-h-14 bg-white border-b border-slate-200 flex items-center px-3 py-2 gap-2 shrink-0 sm:px-4 md:h-14 md:px-6 md:py-0 md:gap-4">
      ...
      <div className="hidden sm:flex items-center gap-2 bg-slate-100 hover:bg-slate-200 transition-colors rounded-lg px-3 py-1.5 w-44 cursor-text">
        <Search size={13} className="text-slate-400 shrink-0" strokeWidth={2.25} />
...
// ── Notification bell ─────────────────────────────────────────────────────────
// Extracted so it can later become a full dropdown component.
...
function NotificationBell() {
  ...
  return (
    ...
    >
      ...
      {unreadCount > 0 && (
        <span className="absolute top-1 right-1 w-1.75 h-1.75 bg-red-500 rounded-full border-[1.5px] border-white" />
      )}
    </button>
  );
}